...
This is the canonical cross-product roadmap for the VINCENT product family. Product-specific implementation details remain in the Vincent and CIC Station product roadmaps.
...
| M8 | Full destructive recovery proves workers and CIC Station can be reconstructed from durable/protected external state; 1.0 program acceptance criteria satisfied | Planned |
...
The program deliberately uses ordinary GitHub repositories, issues, pull requests, milestones, labels, and repository Markdown rather than GitHub Projects v2.

- `logrusbox/vincent-program` holds only cross-product work and program-level coordination.
- `logrusbox/vincent` owns Vincent-specific work.
- `logrusbox/cic-station` owns CIC Station-specific work.
- Product release targeting uses repository milestones in the relevant product repository.
- Cross-product program stage uses M0-M8 in this roadmap and, when useful, program-issue metadata/labels.
- PRs are implementation/review evidence; issues are planning/work anchors.
- Do not create duplicate program issues merely to mirror product issues.

## Cross-product acceptance rules

A program milestone is complete only when:

1. required implementation exists in the owning product repositories;
...
6. cross-product compatibility/authority boundaries are coherent;
7. no unresolved safety or ownership contradiction remains.

## Deferred capabilities

Unscheduled future ideas belong in GitHub issues in the repository that owns them. Promote an item into this program roadmap only when it becomes a concrete cross-product milestone outcome.
